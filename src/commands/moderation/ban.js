...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (bUser.hasPermission("BAN_MEMBERS"))
      return message.channel.send("The user you provided cannot be banned :x:");
    if (!bReason) bReason = "no reason provided";
    bUser.ban({ reason: bReason }).then(() => {
      message.channel.send(`**${bUser.tag}** has been banned for ${bReason}`);
      ...
      bUser.send(`You have been banned in Lighting Ads for ${bReason}`);
    });
  },
};
